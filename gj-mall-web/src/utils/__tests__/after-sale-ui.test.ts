...
describe('after-sale-ui', () => {
  it('keeps action visibility stable across after-sale statuses', () => {
  ...
  it('builds compact refund record text for user-facing cards', () => {
    const refund = {
  ...
  it('returns empty text when there is no refund record yet', () => {
    assert.equal(refundRecordTitle(undefined), '')
    assert.equal(refundRecordMeta({}), '')
  })
