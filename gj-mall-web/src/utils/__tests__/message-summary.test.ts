...
describe('message-summary', () => {
  ...
  it('routes after-sale messages to detail when biz id is present', () => {
    assert.equal(messageNavigationTarget({ bizType: 'after_sale', bizId: 200 }), '/after-sales/200')
    assert.equal(messageNavigationTarget({ bizType: 'after_sale' }), '/after-sales')
    assert.equal(messageNavigationTarget({ bizType: 'order', bizId: 100 }), '/order/100')
  })
